...
```{r deprecated, echo=TRUE, eval = FALSE}
...
The `install()` function lets users download and pull new images on to
...
This function allows the users to check which images are installed on
...
Once some time has passed and if you are not sure if you image is upto
date,
